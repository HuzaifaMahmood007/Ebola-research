...
Across 12 cells, **none is significantly better and one is significantly worse**: US-regions at *h* = 10, at −1.35% with an interval of [−2.52, −0.19]. The other eleven are within noise, and best validation loss is a wash too, with a paired mean of +0.0020 and an interval of [−0.0055, +0.0095]. The inner loop costs about 5% more wall-clock and returns nothing.

Four things bound how far this reads. The episodes come from one disease, so they vary population and forecast origin rather than disease. Both arms warm-start from the transfer trunk, making this meta-fine-tuning rather than meta-learning from scratch. The meta-test fits a fresh adapter on full training folds, so nothing here measures few-shot behaviour. And 5,900 of 8,000 episodes were skipped on mask density, leaving 2,100 usable outer updates.
...
A third result belongs beside them. Meta-learning the adaptation surface does not help either, being better in none of 12 cells and worse in one, so adapting better is not what this problem needs.
