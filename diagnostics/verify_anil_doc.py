...
BESTVAL_RE = re.compile(r"Held-out meta-objective[^:]*:\s*(.+?),\s*(yes|within noise|untestable)\.")





def check_best_val(text):

    """The meta-objective line is PROSE, so no table check touches it.



    It was wrong once, in the direction that matters most: the values were sign-flipped before the

    delta, so every fold printed the opposite of what the artifacts say. Prose gets checked here for
    ...
    found = BESTVAL_RE.findall(text)

    folds_with_tables = [f for f in FOLDS if secs[f]["primary"]]

    if len(found) != len(folds_with_tables):

        problems.append(f"{len(found)} meta-objective lines for {len(folds_with_tables)} folds")

        return problems

    for fold, (delta_txt, label) in zip(folds_with_tables, found):

        got = _parse_delta(delta_txt)

        if got is None:

            problems.append(f"{fold}: unparseable meta-objective delta {delta_txt!r}")

            continue

        pm, _phw, pn, pclears = got

        anil = {s: r["best_val"] for s, r in _rows(fold, "anil").items()

                if r.get("best_val") is not None}

        ctrl = {s: r["best_val"] for s, r in _rows(fold, "control").items()

                if r.get("best_val") is not None}

        # best_val is a loss: ANIL better means a LOWER value, so it is the improvement direction

        # and goes through the same paired() as an error metric.

        m, _hw, n, clears = paired(anil, ctrl)

        if m is None:

            problems.append(f"{fold}: meta-objective printed with no artifacts behind it")

            continue

        if n != pn:

            problems.append(f"{fold}: meta-objective printed n={pn}, recomputed n={n}")

        if abs(m - pm) > _tol_for(f"{pm}"):

            problems.append(f"{fold}: meta-objective printed {pm:+.2f}%, recomputed {m:+.2f}% "

                            f"(a sign flip here reverses the claim)")

        if pclears != clears:

            problems.append(f"{fold}: meta-objective significance printed {pclears}, "

                            f"recomputed {clears}")

        want = "yes" if clears else "within noise"

        if label != want:

            problems.append(f"{fold}: meta-objective label {label!r} should be {want!r}")
...
CHECKS = (("primary table", check_primary),
          ...
          ("meta-objective prose", check_best_val),
...
def mutate(text):
    ...
    # the meta-objective sign reversed -- the defect that actually happened. Both rendered forms
    # must be handled: "within noise (+0.8 ... )" carries no % after the mean, the significant form
    # does, so match the signed number itself rather than assuming a suffix.
    for m in BESTVAL_RE.finditer(text):
        inner = m.group(1)
        m2 = re.search(r"[-+]\d+\.\d+", inner)
        if not m2:
            continue
        flipped = ("-" if m2.group(0)[0] == "+" else "+") + m2.group(0)[1:]
        muts.append(("meta-objective sign reversed",
                     text.replace(inner, inner[:m2.start()] + flipped + inner[m2.end():], 1)))
        break
