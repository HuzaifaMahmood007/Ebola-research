...
### 2.9 COVID-19: excluded in Phase 2, then admitted

**Superseded 2026-09-06.** The paragraph below was true when Phase 2 closed and is no longer true.
It is kept because the reversal, not the original position, is the thing a reader needs to follow.

> Stated for the record: the COVID-19 datasets shipped with the baseline implementations were used
> **only** in the Phase-1 reproduction fidelity checks. The development set is **dengue and
> influenza**. No COVID-19 series enters the harmonised schema and no COVID-19 dataset is released.

**What actually happened.** The client asked for COVID-19 back as a third development disease in the
Week 3 review (`Review Doc.md:97`), and the build followed that instruction. A COVID-19 bundle was
constructed after Phase 2 closed and is audited in full in **§6**: New York Times `us-states.csv`,
**49 nodes, 164 weeks, mask density 1.0000**, per-node training scaler, fixed 50/20/30 chronological
split, on ColaGNN's `state-adj.txt` reused verbatim so it is **bit-identical** to the
`influenza_us-states` graph.

It is a released dataset, it trains the shared trunk behind every result in the study including the
Ebola case study, and the development set is therefore **dengue, influenza and COVID-19**: five
panels, three diseases. Client decision **B5 is amended accordingly** in
`progress/decisions/client_decisions.md`. This closes audit finding M8.
...
- **Contiguity is an induced subgraph.** Guinea reports thirty-two of its thirty-four prefectures, and
  ...
  of geography.
- **Imputed weeks.** At a mask density of **0.4095** (0.5255 before the corrupt weeks were masked,
  §3.5), Ebola contributes by far the most imputed steps of the three diseases. Internal reporting gaps are forward-filled in *cumulative* space and flagged as
...
**Six datasets, three diseases, one weekly schema**: five development panels plus the Ebola
holdout. All are regenerated by a single command ([build_datasets.py](build_datasets.py)).

**Corrected 2026-09-06.** This table previously listed five datasets and omitted COVID-19, and gave
Ebola's mask density as 0.5255. Every figure below was recomputed from `data/processed/*.npz` on
that date; Ebola's real density is **0.4095**.
...
| COVID-19 (US states) | development | 49 | 164 | 1.000 | fixed 50/20/30 | per node, training slice |
| Ebola | **few-shot holdout** | 61 | 52 | **0.4095** | support / query | pooled support cells |

Ebola is the only panel whose scaler is fitted at disease level rather than per node
(`scaler_scope=per_disease_support`). That asymmetry is a covariate shift we introduced ourselves and
it is priced in the manuscript's Threats section, not hidden here.
...
| Development set | **Dengue, influenza and COVID-19**: five panels, three diseases. COVID-19 was excluded when Phase 2 closed and re-admitted at the client's request in the Week 3 review; see §2.9 and §6, and the B5 amendment in `client_decisions.md` |
